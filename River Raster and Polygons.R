#I will create a separate script for the river raster and shapefile data
...
river_lines = readOGR("KML files/Ri)
...
trainData = extract(gis_data, river_lines, buffer=30)
trainData2 = do.call("rbind", trainData)
river = rep(c(1,1,1,0,0,0,0), times=sapply(trainData, nrow))
svmModel = svm(x=trainData2, y=as.factor(river))
